...
theorem not_not_elim {P : Prop} (h : ¬¬P) : P :=
  prop_rec (λ x => ¬¬x → x) (λ _ => trivial) (λ h1 => h1 not_false) h
...
theorem or_elim {P Q R : Prop} (h1 : P ∨ Q) (h2 : P → R) (h3 : Q → R) : R :=
  not_not_elim # λ h4 => h4 # h3 # h1 # modus_tollens h2 h4

theorem and_intro {P Q : Prop} (h1 : P) (h2 : Q) : P ∧ Q :=
  λ h3 => or_elim h3 (λ h4 => h4 h1) (λ h4 => h4 h2)

theorem and_elim_left {P Q : Prop} (h : P ∧ Q) : P :=
  not_not_elim # λ h1 => h # or_intro_left h1

theorem and_elim_right {P Q : Prop} (h : P ∧ Q) : Q :=
  not_not_elim # λ h1 => h # or_intro_right h1
...
(prop_rec (λ x => (true ↔ x) → F true → F x) (λ _ => id)
  (λ h₃ => false_elim # iff_elim_left h₃ trivial))
(prop_rec (λ x => (false ↔ x) → F false → F x)
  (λ h₃ => false_elim # iff_elim_right h₃ trivial) (λ _ => id))
h₁ h₂

theorem eq_refl {α : Type} {x : α} : x = x := λ _ => id

theorem eq_symm {α : Type} {x y : α} (h : x = y) : y = x :=
  not_not_elim # λ h1 => modus_tollens (h (λ z => z = x)) h1 eq_refl

theorem eq_iff {α : Type} {x y : α} : x = y ↔ (∀ P : α → Prop, P x ↔ P y) :=
  iff_intro (λ h P => iff_intro (h P) (eq_symm h P)) (λ h => iff_elim_left (h (λ z => x = z)) eq_refl)


theorem eq_rec {α : Type} (P : α → Prop) {x y : α} (h1 : x = y) (h2 : P x) : P y := h1 P h2
...
theorem or_self {P : Prop} : P ∨ P ↔ P := iff_intro (λ h => or_elim h id id) or_intro_left

theorem or_symm {P Q : Prop} (h : P ∨ Q) : Q ∨ P := or_elim h or_intro_right or_intro_left

theorem and_self {P : Prop} : P ∧ P ↔ P := iff_intro and_elim_left (λ h => and_intro h h)

theorem and_symm {P Q : Prop} (h : P ∧ Q) : Q ∧ P := and_intro (and_elim_right h) (and_elim_left h)

theorem not_or {P Q : Prop} : ¬(P ∨ Q) ↔ (¬P ∧ ¬Q) := sorry

theorem not_and {P Q : Prop} : ¬(P ∧ Q) ↔ (¬P ∨ ¬Q) := sorry

theorem not_iff_not_self {P : Prop} : ¬(P ↔ ¬P) := sorry

theorem not_iff {P Q : Prop} : ¬(P ↔ Q) ↔ (¬P ↔ Q) := sorry
